// Scenario ID: D2 from docs/biz/uses.md — peak 1000 calls.
...
export const options={ scenarios: { peak: { executor:'ramping-vus', stages:[{duration:'5m',target:1000},{duration:'10m',target:1000},{duration:'5m',target:0}] } } };
export default function(){ const traceparent=`00-${__VU.toString(16).padStart(32,'0')}-${__ITER.toString(16).padStart(16,'0')}-01`; const res=http.post(`${__ENV.UDCSP_APIM_BASE_URL||'https://example.invalid'}/voice/ivr/sessions`,JSON.stringify({scenario:'D2',synthetic:true}),{headers:{traceparent,'content-type':'application/json',authorization:`Bearer ${__ENV.UDCSP_TEST_TOKEN||'scaffold'}`}}); latency.add(res.timings.duration); errorRate.add(res.status>=400); check(res,{'status is not 5xx':r=>r.status<500,'trace echoed':r=>(r.headers.Traceparent||traceparent).includes(traceparent.split('-')[1])}); sleep(1); }

export function handleSummary(data) {
  return { [`tests/load/results/${__ENV.UDCSP_RESULTS_DATE || 'local'}/voice-ivr-call-burst.json`]: JSON.stringify(data, null, 2) };
}
