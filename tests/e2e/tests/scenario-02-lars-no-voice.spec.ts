// Scenario ID: D2 from docs/uses.md — Lars Norwegian voice status query.
...
test('Scenario 02 - Lars Norwegian voice status query', async ({ page, request, persona }) => {
  ...
  const submission=await postJson(request,'/gateway/demo-scenarios/d2',{personaId:persona.id,scenario:'D2',intent:'voice-tax-status'},traceparent,token);
