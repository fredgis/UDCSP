...
1. Sign in to <https://make.powerapps.com> with the tenant admin account
   ...
   **UDCSP-Dev** environment (org URL `<your-dataverse-env>.crm4.dynamics.com`).
...
4. **Solutions → Default Solution (or new solution `UDCSP Caseworker`) → + New → App → Model-driven app**,
   ...
   *(The legacy "Apps → New → Model-driven" entry was removed from the
   ...
   > `d365CasesEndpoint=https://<your-dataverse-env>.crm4.dynamics.com/api/data/v9.2/tasks`,
...
az resource update -n udcsp-dk-dev-application-intake -g udcsp-dk-logicapps-rg `
  ...
  --set properties.parameters.d365CasesEndpoint.value="https://<your-dataverse-env>.crm4.dynamics.com/api/data/v9.2/udcsp_applications"
