...
5. **Eligibility Pre-Assessor** (Foundry, EU AI Act high-risk) computes provisional residency entitlement and triggers a **mandatory human review** flag.
...
10. Anna receives the notification on her phone within 4 days; clicks through to the **SE portal** and is auto-onboarded via the federated identity.
...
| Automated eligibility pre-assessment | #7 | Eligibility model returns a provisional decision + confidence + explanation. |
| GDPR + EU AI Act + sector compliance | #9 | Eligibility decision logged into AI Act registry; DPIA reviewed. |
...
- 💬 *"Watch the Power BI tile — average residency-transfer SLA is now 4 days, against a 28-day baseline."*
...
| 🌐 **Channels** | Telephone (PSTN → ACS → Copilot Studio → Foundry). |
...
2. **Copilot Studio** voice agent greets in NB, captures intent ("tax refund why so low this year?").
3. Copilot Studio invokes the **Citizen Assistant agent** in Foundry; the agent retrieves the relevant tax-rule article from the multilingual knowledge base in Fabric.
...
| GenAI citizen assistant on telephone | #6 | Voice assistant powered by Foundry + Copilot Studio + AI Speech. |
...
- **Additional:** Microsoft Foundry, Copilot Studio, Azure Communication Services, AI Speech, AI Content Safety, Application Insights.
...
7. He receives an in-app notification within 4 days with the final decision; a Power BI tile shows him the average decision time for his case type.
...
3. **D365 plugin** writes the override to Dataverse, mirrored to **Fabric audit lakehouse**, also tagged in the **Foundry trace** as `human-override`.
4. Override + reason flow into the **AI Act registry** in Purview; this case is added to the **shadow-mode evaluation** sample for next month.
5. Astrid issues the favourable decision; the citizen receives a positive notification.
6. A Power BI tile updates: **% AI overrides this week**, broken down by case type and reason.
...
- **Additional:** Foundry tracing, AI Act registry tooling, Application Insights.
...
| 🌐 **Channels** | Power BI audit dashboard + Foundry trace explorer + Purview catalog. |
| 🌍 **Languages** | DA / EN. |
| ⏱️ **Duration** | ~ 8 min |
...
5. Hans simulates a **DSAR (Data Subject Access Request)**: a click produces a citizen-facing export including the AI decision rationale in DA.
...
| 🌐 **Channels** | Power BI executive workspace. |
...
- **Additional:** Microsoft Foundry, Static Web Apps, ACS, Copilot Studio, Container Apps, Functions, Cosmos DB, Key Vault, Bicep, GitHub Actions, AI Document Intelligence, AI Translator, AI Speech, Sentinel, Defender for Cloud, Application Insights.
- **Foundry agents:** all six imported and evaluated.
