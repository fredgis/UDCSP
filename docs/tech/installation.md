...
- **Self-service trial (recommended, 1 minute):**
  1. Sign in to <https://trials.dynamics.com> with your tenant admin account (`admin@MngEnvMCAP123456.onmicrosoft.com` in the example).
...
2. Click the environment row (e.g. the one matching `<your-dataverse-env>.crm4.dynamics.com`).
...
These are the issues we hit on a fresh `MngEnvMCAP123456.onmicrosoft.com` MCAPS sandbox and the fixes now baked into the installer. **You should not have to redo these** — they are documented so you understand the `[skip]` lines you will see in the logs.
...
$upn = 'admin@MngEnvMCAP123456.onmicrosoft.com'
...
Caseworkers are **not** citizens — their identity lives in the **main Entra tenant** `MngEnvMCAP654321.onmicrosoft.com`, not in the per-country External ID tenants. A single caseworker can be granted access to the DK / SE / NO Dataverse environments and triage cases from any country.
...
.\apps\powerapps\caseworker\bootstrap-udcsp-application.ps1 `
  -EnvUrl https://<your-dataverse-env>.crm4.dynamics.com
...
#    b. Publish. Bookmark
#       https://<your-dataverse-env>.crm4.dynamics.com/main.aspx?appid=<APP_GUID>
```
...
```powershell
.\apps\powerapps\caseworker\deploy.ps1 `
  -SourceEnv https://<your-dataverse-env>.crm4.dynamics.com `
  -TargetEnvs @(
