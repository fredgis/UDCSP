...
> - **Invocation = OpenAI Responses API.** Agents expose the `responses` protocol at `POST {projectEndpoint}/openai/v1/responses` with body `{model, instructions, input}`. Two call patterns are in use today:
>   - **APIM operation policies (SPA-facing path)** — the citizen ChatWidget hits APIM `/agent-classifier`, `/agent-eligibility`, `/agent-doc-extractor`. Each operation policy authenticates with APIM's system MI on `https://ai.azure.com`, transforms the SPA payload into `{model, instructions, input}` (instructions stored as APIM named values, sourced from `foundry/agents/*/system-prompt.md`), and forwards. JWT validation runs first.
