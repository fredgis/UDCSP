...
> [!WARNING]
> **Security remediation status, 2026-08-11.** The gateway, installer, diagnostic-setting, allowed-region, browser-cache and Logic Apps host-storage fixes described below are 🔵 **In repo** or ⚙️ **Scripted**, not deployed. The live APIM instances still serve the previously published policies, the live APIM managed identity still has an account-scoped storage grant, and the new Storage and Key Vault diagnostic settings do not exist in Azure. APIM diagnostics remain an open gap.
...
6. The SPA calls APIM with that `access_token`. The External ID policy validates the signing chain and issuer metadata, pins `aud` to `{{external-id-api-audience}}`, and requires `scp` to contain `access_as_user`. For citizen Logic App routes, APIM derives the citizen UPN from the validated token, deletes any caller-supplied `x-udcsp-citizen-upn`, writes the trusted header, and then routes to the DK backend. Body fields such as `citizenId`, `citizen.upn`, `email` and `subjectId` are not identity authorities.
...
- **Foundry continuity.** Because there are 3 hubs, the loss of one region does not stop the platform globally: a failed `norwayeast` hub only impacts NO citizens. Per-country cross-region failover is not currently executable under the checked-in allowed-regions initiative, which admits only `northeurope` for DK, `swedencentral` for SE and `norwayeast` for NO. An approved recovery region and corresponding policy change are prerequisites for regional failover.
...
- **API Management** policies enforce country routing, OAuth scope checks, rate limiting per channel, request transformation and body redaction for logs. 🔵 **In repo:** citizen-facing Logic App routes derive identity from the validated token and forward only the gateway-owned `x-udcsp-citizen-upn` header. Caller-supplied copies are deleted first, and identity-looking body fields are non-authoritative.
...
- **Network target:** hub-and-spoke per sovereign zone, peered via the federation hub VNet, with **Private Endpoints** on PaaS services. No public ingress except **Azure Front Door Premium + WAF** (Microsoft `DefaultRuleSet 2.1`, `MicrosoftDefaultRuleSet 1.0` for bot protection, and a tenant-managed `RateLimitRuleSet` per citizen IP) and APIM's external gateway. **Azure DDoS Protection Standard** is bound to VNets that front an approved public IP for L3/L4 defence in depth. Deployment status is resource-specific; the Logic Apps host-storage blob and file endpoints are source-only.
- **Sovereign region policy:** 🔵 **In repo:** the allowed-regions initiative binds DK to `northeurope`, SE to `swedencentral` and NO to `norwayeast` for both resources and resource groups. It no longer allows `westeurope` for citizen-data resources. The revised initiative is not deployed, and its single-region allow-list must be extended deliberately before any cross-region DR deployment.
...
- **Private DNS:** Per-country **Private DNS Zones** serve the deployed Private Endpoint surfaces. The private-upload patch creates and links `privatelink.blob.core.windows.net`. Other target zones include `privatelink.dfs.core.windows.net`, `privatelink.file.core.windows.net`, `privatelink.vaultcore.azure.net`, `privatelink.postgres.database.azure.com`, `privatelink.redisenterprise.cache.azure.net`, `privatelink.azurecr.io`, `privatelink.cognitiveservices.azure.com` and `privatelink.confidential-ledger.azure.com`. The new Logic Apps host-storage Private Endpoints do not create DNS zones or zone groups in `workspace.bicep`, so blob and file DNS resolution must be supplied and verified during deployment.
...
- **Gateway authentication and trusted identity:** 🔵 **In repo:** every API directory has a mandatory `policy.xml`. External ID policies pin the API audience and require `access_as_user`; workforce APIs use the Entra fragment. `agent-topic-router` and `agent-doc-extractor` are no longer anonymous in source. `lineage` is authenticated and returns `503` until a backend exists. These policies are not live until the APIM phase is rerun.
...
- **Logic Apps host storage:** 🔵 **In repo:** `services/logic-apps/workspace.bicep` disables public network access and shared-key authorization, uses `AzureWebJobsStorage__accountName` plus managed identity instead of an embedded account key, and creates blob and file Private Endpoints. The live workspace has not received this change.
- **Upload least privilege:** ⚙️ **Scripted:** future APIM installs create `citizen-uploads` before RBAC and grant `Storage Blob Data Contributor` at that container only. The existing live account-scoped grant is not revoked by the installer change and requires a separate manual removal.
...
- **Document upload validation:** 🔵 **In repo:** APIM enforces an 8 MiB decoded ceiling, accepts only PDF, PNG and JPEG extensions with matching declared types, checks decoded magic bytes, and writes a server-derived blob content type. The browser limit is usability only, not the security boundary.
- **Resource diagnostics:** 🔵 **In repo:** blob `StorageRead`, `StorageWrite` and `StorageDelete` events plus Key Vault `AuditEvent` and policy-evaluation events route to the country Log Analytics workspace. These settings are not deployed. APIM diagnostic settings are still missing.
...
- **BCDR, Backup and Site Recovery:** 🔵 **In repo:** backup vault, policy and ASR scaffolding exists per country. The revised allowed-regions initiative currently permits only each country's primary region, so an active-passive secondary-region deployment and DNS cut-over are not deployable under that policy. Current executable recovery is in-region restore and PITR; regional failover requires an approved recovery region plus a coordinated policy update.
- **Resilience proof, Chaos engineering:** **Azure Chaos Studio** experiments target the citizen-facing path (Front Door → APIM → Container Apps → Postgres → Foundry hub). In-region faults can run on the planned monthly non-prod and quarterly prod cadence. Regional failover remains a table-top scenario until the allowed-regions policy admits an approved recovery region.
...
🔵 **In repo, not live:** landing-zone Storage and Key Vault diagnostics now target the country LAW. APIM diagnostics remain absent, so APIM gateway events do not yet have the complete forensic path described by the target model.
...
| Case Management | D365 Customer Service *(roadmap — currently a model-driven Power App on the shared Dataverse env `<your-dataverse-env>` until per-country D365 CS envs are provisioned; same `udcsp_application` schema for drop-in replacement)* | Multilingual KB; per-language SLA queues; caseworker UI in their working language; outbound mail templates per locale, edited before send. |
...
sequenceDiagram
    ...
    Web->>APIM: POST /gdpr/erasure-request + bearer token
    Note over Web,APIM: Subject comes from validated token, not request body
...
3. APIM `validate-jwt` policies use the External ID discovery document, pin `{{external-id-api-audience}}`, and require `scp=access_as_user`. 🔵 **In repo:** every API directory now has `policy.xml`. External ID protects `citizen-applications`, `documents`, `eligibility-checks`, `data-export`, `gdpr`, `agent-topic-router` and `agent-doc-extractor`; workforce Entra protects `case-management`, `notifications`, `citizen-insights`, `agent-citizen-assistant`, `agent-classifier` and `lineage`. The new `lineage` policy fails closed with `503` because no backend exists. The live gateways still have the previously published policies until redeployed.
...
| **Azure Backup + Azure Site Recovery** | Per-country backup and recovery scaffolding for Postgres, Redis, critical Storage and supported runtimes. RPO/RTO values are targets; cross-region execution is blocked until policy admits an approved recovery region. *(Post-audit addition; previously absent.)* Bicep at [`infra/security/backup-asr/`](../../infra/security/backup-asr/). |
...
| **Azure Chaos Studio** | Resilience experiments for in-region faults, including NSG isolation, Postgres failover and a per-country Foundry hub blackout. Regional failover is table-top only until policy admits an approved recovery region. *(Post-audit addition.)* Bicep at [`infra/security/chaos-studio/`](../../infra/security/chaos-studio/). |
...
| **Fail closed on API policy gaps** | ⚙️ **Scripted:** `Install-Apim.psm1` throws before API import if any directory with `openapi.yaml` lacks `policy.xml`; it will not silently publish an unauthenticated API. |
| **Secret-safe command logging** | ⚙️ **Scripted:** known secret-bearing command arguments, including Static Web Apps deployment tokens, are replaced with `<redacted>` before the phase command line is written to `scripts/install/reports/`. |
...
| **Azure Backup + Azure Site Recovery** | Per-zone backup baseline with PostgreSQL PITR, ADLS recovery controls and ASR policy scaffolding. Cross-region replication is blocked by the current single-region country allow-list until an approved recovery region is added to policy. | `BackupAsr` |
