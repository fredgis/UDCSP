...
> [!WARNING]
> **Security remediation status, 2026-08-11.** The browser-cache minimisation, gateway identity, upload validation, safe blob deletion, Storage diagnostics, Key Vault diagnostics and Logic Apps host-storage changes are 🔵 **In repo** or ⚙️ **Scripted**, not deployed. Live Azure and the live SPA retain their previous posture until the relevant APIM, infrastructure and web deployments are run.
...
| **D7** | **Right to erasure is a service, not a project** | The self-service entry point is `POST /gdpr/erasure-request`. APIM derives the citizen from the validated token; a caller-supplied subject identifier cannot select another person's records. The target executor cascades deletion across all 5 zones within the GDPR Art. 12(3) SLA. |
| **D8** | **Audit trail target is immutable** | 🔵 **In repo:** Storage and Key Vault diagnostics add source events for the country LAW, but they are not deployed and APIM diagnostics remain missing. Immutable retention is the target once collection is complete. |
...
> mirrors that fallback by querying `tasks` for the exact delimited
> `citizenUpn: <upn>` ownership token. The claim is escaped before it is
> inserted into the OData filter. The application-intake Logic App writes
> the UPN only from APIM's trusted `x-udcsp-citizen-upn` header, never from
> `citizenId`, `citizen.upn`, `email` or another identity-looking body field.
> This gives
...
> 🔵 **In repo**, not live: browser cache minimisation keeps `localStorage` key
> `udcsp.cases.v1` keeps only the minimal offline case index: case ID, title,
> status, update time, country, citizen UPN, application type, estimated date
> and workflow-step status/timestamps. Extracted document fields, blob URLs
> and names, storage-account names, eligibility snapshots, confidence and
> decisions remain in the current SPA's memory only and are re-hydrated from
> APIM after reload. On read, legacy cache records are rewritten through this
> allow-list, which scrubs previously persisted rich fields.
...
> 🔵 **In repo**, not live: the document operation accepts
> only PDF, PNG and JPEG (`.jpg` or `.jpeg`), rejects decoded content above
> 8 MiB, verifies the decoded magic bytes and stores a server-derived content
> type. For citizen case deletion, APIM accepts only a stored blob name matching
> `YYYYMMDD/<32 lowercase hex>_<safe filename>`, then rebuilds the URL from the
> configured storage account and `citizen-uploads` container. It never sends a
> managed-identity token to an arbitrary caller-supplied URL.
...
| 3a | Browser offline case index | `localStorage` key `udcsp.cases.v1`, minimal allow-listed metadata only | Client edge, not a system of record | ❌ No | Per signed-in citizen and selected country |
...
| 25 | APIM telemetry | No complete diagnostic stream is configured today | Cross-cutting control gap | ❌ No guaranteed mirror | Per-country APIM exists, diagnostics remain open |
...
| Browser offline case index | Up to 50 minimal records; cleared on account change, sign-out or citizen erasure | None | Allow-list rewrite scrubs legacy rich fields on read | GDPR Art. 5(1)(c) |
...
| APIM telemetry | Diagnostic settings are still missing | None guaranteed | Open remediation item | Operational and forensic gap |
| Key Vault audit | 🔵 **In repo:** `AuditEvent` routes to country LAW; not deployed | None until deployment | Per LAW/Sentinel policy after deployment | Operational |
...
| **GDPR** | Art. 32: security of processing | Encryption + pseudonymisation + resilience | CMK + per-citizen pseudonymisation in OneLake Silver + country-lake GRS and service-specific recovery controls (§ 10) |
...
GDPR Art. 17 demands a way to delete a citizen's personal data on request, within 30 days, with proof. 🔵 **In repo, not deployed:** the self-service APIM endpoint authenticates the citizen and forwards the gateway-owned `x-udcsp-citizen-upn` identity. Body fields such as `subjectId` are not authoritative. The full Priva and five-zone cascade below remains the target operating model.
...
sequenceDiagram
    ...
    participant API as APIM /gdpr/erasure-request
    ...
    CIT->>API: POST /gdpr/erasure-request + bearer token
    Note over CIT,API: Subject comes from validated token, not request body
...
| Store | Backup strategy | RPO | RTO | Recovery locality and constraint |
|---|---|---|---|---|
| Dataverse | Native long-term backup, point-in-time restore | 1 h. | 4 h. | Service-managed recovery; any explicit secondary deployment needs a policy-approved region |
| PostgreSQL Flexible Server | Azure Backup + Flexible Server PITR + service-managed replicated backups | 15 min. | 1 h. | In-region zone resilience; restoring a server in another region requires that region in policy |
| Azure Cache for Redis (Enterprise) | RDB export to country Storage | 15 min. | 30 min. | Uses the country storage recovery posture; no deployable secondary region is currently allowed |
| Confidential Ledger | CCF-managed multi-node consensus replication | 0 (synchronous) | < 5 min. | In-region multi-node; a second-region ledger is target-only until policy admits that region |
| Confidential Container Apps | Stateless; image retained in ACR | N/A (stateless) | 10 min. | Same-region IaC redeploy is executable; cross-region ACR replication and redeploy need policy approval |
| ADLS Gen2 country lake | `Standard_GRS` in the checked-in LandingZone module; additional soft-delete, versioning and vault protection require separate configuration | Service-dependent | 1 h. target | Geo-redundant account replication |
| Logic Apps host storage | `Standard_LRS`; redeployable from IaC, with identity-based access and blob/file Private Endpoints 🔵 **In repo** | Workflow checkpoint dependent | Recreate and restore per workflow plan | In-region only |
...
**BCDR ownership.** Backup and ASR scaffolding is in [`infra/security/backup-asr/`](../../infra/security/backup-asr/). The checked-in allowed-regions initiative now permits only the primary sovereign region for each country, so cross-region restore and ASR failover require an approved recovery region and a coordinated policy update first. Until then, the executable path is in-region restore and PITR. Chaos Studio definitions remain in [`infra/security/chaos-studio/`](../../infra/security/chaos-studio/).

**DR drill cadence.** Twice yearly per country, using a regional-failover table-top plus an executable same-region restore with caseworker simulation; see `docs/tech/runbook-dr.md`.
...
The forensic path has a mixed deployment state:

- 🔵 **In repo:** Key Vault `AuditEvent` and `AzurePolicyEvaluationDetails` route to the country Log Analytics workspace.
- 🔵 **In repo:** blob `StorageRead`, `StorageWrite` and `StorageDelete` events route to the same workspace.
- These Storage and Key Vault settings are not deployed, so they produce no live evidence yet.
- APIM diagnostic settings are still missing. Claims that all cross-border APIM calls reach Sentinel are not currently supportable.
- Erasure and human-override application events remain part of the target Sentinel detection model.
...
| Persisting extracted fields, blob URLs or AI verdicts in browser storage | Expands the client-side PII and decision footprint across reloads | Keep rich case state in memory only; persist a minimal allow-listed case index and scrub legacy cache records on read |
...
> *UDCSP assigns citizen data to five bounded zones with documented retention and country federation. The repository now minimises browser persistence and adds Storage and Key Vault diagnostic settings, but those controls are not live and APIM diagnostics remain open. Zone 3, Conversations, formalises where dialog, voice, SMS, email and AI traces are intended to live.*
