...
Today's demo persists cases in the generic Dataverse `task` activity entity because `<your-dataverse-env>.crm4.dynamics.com` has **no `incident` table** (Customer Service is not installed). Once the licence is provisioned and Customer Service is installed in this environment, swap the demo back to the proper case model.
...
| Power Apps maker (apps + solutions list) | https://make.powerapps.com (top-right env picker → select `<your-dataverse-env>`) |
...
| Dataverse model-driven apps for this env | https://<your-dataverse-env>.crm4.dynamics.com/main.aspx?forceUCI=1&pagetype=apps |
| Dynamics 365 home (app launcher) | https://home.dynamics.com |
| Web API root (sanity check `incidents` shows up after install) | https://<your-dataverse-env>.crm4.dynamics.com/api/data/v9.2/ |

**Install Customer Service:** Power Platform admin → *Resources → Dynamics 365 apps → Install app → Customer Service Hub* (target env `<your-dataverse-env>`). After install, `incident`, `case`, `customerservicestakeholder`, `queueitem`, etc. become available.
...
1. **Logic App `udcsp-dk-dev-application-intake`** — `Create_D365_case` action:
   ...
   - Stored param `d365CasesEndpoint` → `https://<your-dataverse-env>.crm4.dynamics.com/api/data/v9.2/incidents`.
