...
3. **D365 connection**: `Create_D365_case` action needs a Dataverse API connection (or service principal + bearer to `https://<your-dataverse-env>.crm4.dynamics.com/api/data/v9.2`). The `d365-dataverse-url` named value is set but the Logic App still uses the old HTTP action with no auth.
